...
function to_next_level (level_number=operating_level_number + 1) {
    operating_level_number = level_number;
    players = players.filter(player2 => {
        if (player2.size >= 48) {
            riwni1[operating_level_number][0] = riwni1[operating_level_number][0].filter(wall => {
                const is_touch = !checkCollision(wall, player2);
                if (!is_touch) riwni1[operating_level_number][3].push(wall);
                return is_touch;
            });
        }
        return !riwni1[operating_level_number][0].some(wall => checkCollision(wall, player2));
    });
    operating_level = riwni1[operating_level_number];
}

function draw_destroyed_walls() {
    ctx.fillStyle = 'gray';
    operating_level[3].forEach(wall => {
        ctx.fillRect(...wall);
    });
}
...
function keysManager(player=player) {
    if (keys[0] == "KeyR" && players.length == 1) 
        restart(player);
}
        ...
        players.forEach(player2 => {
            if (checkCollision(wall, player2)) {
                ...
                if (operating_level[2].length == 0)
                    operating_level[2].push([34, 34, 48, 48]);
            }
        });
    }
}
